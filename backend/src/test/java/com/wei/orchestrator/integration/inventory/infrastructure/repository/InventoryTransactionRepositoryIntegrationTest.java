...
class InventoryTransactionRepositoryIntegrationTest {
    ...
    void shouldFindTransactionsByStatus() {
        InventoryTransaction transaction1 =
                InventoryTransaction.createReservation("ORDER-005", "SKU-001", "WH-01", 10);
        repository.save(transaction1);

        InventoryTransaction transaction2 =
                InventoryTransaction.createReservation("ORDER-006", "SKU-002", "WH-01", 15);
        InventoryTransaction saved2 = repository.save(transaction2);
        saved2.markAsReserved(ExternalReservationId.of("EXT-RES-006"));
        repository.save(saved2);

        List<InventoryTransaction> pendingTransactions =
                repository.findByStatus(TransactionStatus.PENDING);
        assertTrue(pendingTransactions.size() >= 1);
        assertTrue(
                pendingTransactions.stream()
                        .anyMatch(t -> t.getSourceReferenceId().equals("ORDER-005")));

        List<InventoryTransaction> completedTransactions =
                repository.findByStatus(TransactionStatus.COMPLETED);
        assertTrue(
                completedTransactions.stream()
                        .anyMatch(t -> t.getSourceReferenceId().equals("ORDER-006")));
    }
    ...
    void shouldFindTransactionsByType() {
        ...
        InventoryTransaction inboundTransaction =
                InventoryTransaction.createInboundTransaction(
                        "PUTAWAY-002", TransactionSource.PUTAWAY_TASK_COMPLETED, "WH-01", lines);
        repository.save(inboundTransaction);

        InventoryTransaction outboundTransaction =
                InventoryTransaction.createOutboundTransaction(
                        "PICKING-002",
                        TransactionSource.PICKING_TASK_COMPLETED,
                        "WH-01",
        ...
        List<InventoryTransaction> inboundTransactions =
                repository.findByType(TransactionType.INBOUND);
        assertTrue(
                inboundTransactions.stream()
                        .anyMatch(t -> t.getSourceReferenceId().equals("PUTAWAY-002")));

        List<InventoryTransaction> outboundTransactions =
                repository.findByType(TransactionType.OUTBOUND);
        assertTrue(
                outboundTransactions.stream()
                        .anyMatch(t -> t.getSourceReferenceId().equals("PICKING-002")));
    }
    ...
    void shouldFindTransactionsByWarehouseId() {
        ...
        InventoryTransaction transaction1 =
                InventoryTransaction.createInboundTransaction(
                        "PUTAWAY-003", TransactionSource.PUTAWAY_TASK_COMPLETED, "WH-ALPHA", lines);
        repository.save(transaction1);

        InventoryTransaction transaction2 =
                InventoryTransaction.createInboundTransaction(
                        "PUTAWAY-004", TransactionSource.PUTAWAY_TASK_COMPLETED, "WH-ALPHA", lines);
        repository.save(transaction2);

        InventoryTransaction transaction3 =
                InventoryTransaction.createInboundTransaction(
                        "PUTAWAY-005", TransactionSource.PUTAWAY_TASK_COMPLETED, "WH-BETA", lines);
        repository.save(transaction3);

        List<InventoryTransaction> whAlphaTransactions = repository.findByWarehouseId("WH-ALPHA");
        assertEquals(
                2,
                whAlphaTransactions.stream()
                        .filter(t -> t.getWarehouseLocation().getWarehouseId().equals("WH-ALPHA"))
                        .count());

        List<InventoryTransaction> whBetaTransactions = repository.findByWarehouseId("WH-BETA");
        assertTrue(
                whBetaTransactions.stream()
                        .anyMatch(t -> t.getSourceReferenceId().equals("PUTAWAY-005")));
    }

    @Test
    void shouldFindTransactionsByExternalReservationId() {
        List<TransactionLine> lines = new ArrayList<>();
        lines.add(TransactionLine.of("SKU-001", 10));
        ...
        InventoryTransaction transaction =
                InventoryTransaction.createOutboundTransaction(
                        ...
                        "WH-01",
                        lines,
                        ExternalReservationId.of("EXT-RES-UNIQUE-007"));
        repository.save(transaction);

        List<InventoryTransaction> found =
                repository.findByExternalReservationId("EXT-RES-UNIQUE-007");
        assertEquals(1, found.size());
        assertEquals("ORDER-007", found.get(0).getSourceReferenceId());
    }
    ...
    void shouldDeleteTransactionById() {
        InventoryTransaction transaction =
                InventoryTransaction.createReservation("ORDER-008", "SKU-001", "WH-01", 10);
        InventoryTransaction savedTransaction = repository.save(transaction);

        assertTrue(repository.existsById(savedTransaction.getTransactionId()));

        repository.deleteById(savedTransaction.getTransactionId());

        assertFalse(repository.existsById(savedTransaction.getTransactionId()));
        Optional<InventoryTransaction> deleted =
                repository.findById(savedTransaction.getTransactionId());
